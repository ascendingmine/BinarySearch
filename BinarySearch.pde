public class Sketch {
  ...
  public int recursiveLinearSearch(int catNumToFind, int startIndex){
    ...
    if (store[startIndex].getCatalogNumber() == catNumToFind) {
      return store[startIndex].getInStock();
    }
  ...
  public int binarySearch(int catNumToFind){
    ...
    while (low <= high) {
      int mid = (low + high) / 2;
      int midCat = store[mid].getCatalogNumber();

      if (midCat == catNumToFind) {
        return store[mid].getInStock();
      } else if (midCat < catNumToFind) {
        low = mid + 1;
      } else {
        high = mid - 1;
      }
